...
public class Mesh {
  ...
  private void set(int ox, int oy, Mesh next) {
    ...
    double minZ = Mesh.getVertexCaseAt(ox, oy).getMinZ(next, this, ox, oy);
    double maxZ = Mesh.getVertexCaseAt(ox, oy).getMaxZ(next, this, ox, oy);
    ...
    next.setDownhill(ox, oy, Mesh.getVertexCaseAt(ox, oy).getDirection(this, ox, oy));
  ...
  private void computeDirection(int x, int y) {
    ...
    for (short d = 0; d < 8; d++) {
      final double focusZ = getZOrZero(x + dx[d], y + dy[d]);
      if (focusZ < minZ) {
        downhill[x][y] = d;
        minZ = focusZ;
      }
  ...
  private double getZOrZero(int x, int y) {
    if (inBounds(x, y)) {
      return getZ(x, y);
    }
    else {
